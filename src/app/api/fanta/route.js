...
import { FANTA_FEEDS } from "@/lib/config";
...
const CACHE_MS = 5 * 60 * 1000; // 5 minuti
...
async function fetchFeed(source) {
  try {
    ...
    console.error(`[fanta] ${source.id}: ${err.message}`);
    return [];
  }
}
...
export async function GET() {
  ...
  try {
    const results = await Promise.all(FANTA_FEEDS.map(fetchFeed));
